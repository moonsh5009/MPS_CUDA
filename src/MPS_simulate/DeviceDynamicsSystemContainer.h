...
#include "../MPS_database/DynamicsSystem.h"
...
struct DeviceDynamicsSystemData
{
...
class __MY_EXT_CLASS__ DeviceDynamicsSystemContainer : public simulate::DeviceContainer<DynamicsSystem>
{
public:
	using DeviceData = DeviceDynamicsSystemData;

	DeviceDynamicsSystemContainer(ISimulateManager* pSimulateManager);
	...
	{
		DeviceData data;
		data.info = simulate::DeviceContainer<DynamicsSystem>::GetDeviceData();
	...
	mcuda::HostSingleArray<IndexType> maxIterationsArray;
	mcuda::HostSingleArray<double> toleranceArray;

	size_t totalNodeCount = 0;
	std::vector<size_t> entryNodeOffsets;
	std::vector<size_t> entryNodeCounts;
